...
def _tool_result_from_cache(value: object, *, cache_hit: bool) -> dict[str, object]:
    if isinstance(value, dict):
        return {**value, "cache_hit": cache_hit}
    return {"error": "缓存内容异常，请稍后重试。", "cache_hit": cache_hit}
...
def web_search(query: str, max_results: int = 5) -> dict[str, object]:
    ...
    try:
        ...
        lookup = cache.get_or_set_json(
            ...
            should_cache_negative=lambda value: (
                isinstance(value, dict) and value.get("results") == []
            ),
            should_cache=lambda value: (
                isinstance(value, dict) and not value.get("error")
            ),
        )
        result = _tool_result_from_cache(lookup.value, cache_hit=not lookup.created)
        logger.info("External tool completed tool=web_search")
        return result
    ...
    try:
        ...
        lookup = cache.get_or_set_json(
            ...
            should_cache_negative=lambda value: (
                ...
                and str(value.get("error", "")).startswith("未找到地点")
            ),
            should_cache=lambda value: (
                isinstance(value, dict) and not value.get("error")
            ),
        ...
        return _tool_result_from_cache(lookup.value, cache_hit=not lookup.created)
    except Exception as exc:
    ...
    try:
        ...
        lookup = cache.get_or_set_json(
            ...
            should_cache_negative=lambda value: (
                isinstance(value, dict)
                and (
                    str(value.get("error", "")).startswith("股票代码无效")
                    or str(value.get("error", "")).startswith("未找到股票行情")
                )
            ),
            should_cache=lambda value: (
                isinstance(value, dict) and not value.get("error")
