...
export function ThreadIdCopyable({
  ...
  return (
    <TooltipIconButton
      ...
      variant="ghost"
      tooltip="复制线程编号"
      className="border-border hover:bg-muted/40 flex w-fit flex-grow-0 cursor-pointer items-center gap-1 rounded-md border-[1px] p-1"
    >
      <p className="font-mono text-xs">{showUUID ? threadId : "线程编号"}</p>
      <AnimatePresence
        mode="wait"
        initial={false}
      >
