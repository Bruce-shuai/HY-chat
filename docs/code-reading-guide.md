...
- `app/agents/graph.py` 是 `/coding-agent/*` 接口使用的代码分析工作流。它按“扫描文件、搜索、读取、制定计划、总结”的固定顺序运行。
...
6. Agent 按需从 `app/tools/registry.py` 取得工具，再进入 RAG、文件、图片或外部 API 模块。
...
| 修改文件/图片存储 | `app/services/` | `app/storage/service.py`、对应 router |
